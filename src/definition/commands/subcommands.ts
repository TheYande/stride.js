...
>(
  ...
  command: (
    ...
    options: {
      [S in keyof Options]: {
        subcommand: S;
        options: OptionsToType<InGuild, Options[S]>;
      };
    }[keyof Options],
  ) => any,
): void;
...
export function defineSubcommands(
  ...
  if (oldCommand && oldCommand.command !== command)
    throw new ReferenceError(
      "Command " + data.name + " has already been defined",
    );
...
export function transformSubcommands(
  ...
  return Object.entries(subcommands).map(
    ([subcommand, command]: [string, (typeof subcommands)[string]]) => ({
      ...
      options:
        command.options &&
        transformOptions(command.options, { ...metadata, subcommand }),
    }),
  );
}
...
> = {
  options?: never;
  subcommands: {
    [key in keyof Options]: Omit<
      RootCommandData<InGuild, Options[key]>,
      BaseCommandKeys
    >;
  };
