...
export const buttons: Record<
  string,
  (interaction: ButtonInteraction, id: string) => any
> = {};
export function defineButton(
  ...
  if (buttons[buttonName])
    throw new ReferenceError(
      "Handler for button " + buttonName + " already exists",
    );
...
export const modals: Record<
  string,
  (interaction: ModalSubmitInteraction, id: string) => any
> = {};
export function defineModal(
  ...
  if (modals[modalName])
    throw new ReferenceError(
      "Handler for modal " + modalName + " already exists",
    );
...
export function defineSelect(
  ...
  if (selects[selectName])
    throw new ReferenceError(
      "Handler for select menu " + selectName + " already exists",
    );

  const types = typeof typesOrHandler === "function" || [typesOrHandler].flat();
  const handler =
    typeof typesOrHandler === "function" ? typesOrHandler : optionalHandler;
  if (!handler)
    throw new TypeError("No handler passed in for select menu " + selectName);
