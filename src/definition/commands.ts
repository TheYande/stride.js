  ...
  return Object.entries(options)
    .map(([name, option]) => {
      ...
      if (option.autocomplete) {
        ...
        ] ??= {})[metadata.subcommand || NoSubcommand] ??= {})[name] =
          option.autocomplete;
      ...
      if (option.maxLength !== undefined)
        transformed.maxLength = option.maxLength;
      if (option.minLength !== undefined)
        transformed.minLength = option.minLength;
...
export type ApplicationCommandData = {
  ...
  CommandData<boolean>,
  "description" | "type" | "options" | "defaultMemberPermissions"
>;
...
export type BaseGuildCommandData = {
  name: string;
  restricted?: boolean;
  access: (guildId: string) => Promise<boolean> | boolean;
};
